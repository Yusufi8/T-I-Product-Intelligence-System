...
class PurchaseOrder(models.Model):
    ...
    def action_open_pis_product_line_wizard(self):
        self.ensure_one()
        if not self.id:
            raise UserError(_("Save the purchase order before adding governed products."))
        category = False
        sibling_categories = self.order_line.filtered("product_id").mapped("product_id.product_tmpl_id.ti_category_id")
        if len(sibling_categories) == 1:
            category = sibling_categories.id
        return self.env["ti.product.creation.wizard"].with_context(default_category_id=category).action_open_from_context(
...
class PurchaseOrderLine(models.Model):
    ...
    def action_open_pis_product_wizard(self):
        ...
        if not self.order_id:
            raise UserError(_("Save the purchase order before adding governed products."))
        return self.order_id.action_open_pis_product_line_wizard()
