...
def build_standard_product_name(category, spec_lines=None, brand=None):
    """Build the approved PIS product name from category, ordered specs, and brand."""
    parts = []
    if category:
        parts.append(category.name)
    for line in (spec_lines or []):
        value = line.display_value
        if value:
            parts.append(value)
    if brand:
        parts.append(brand)
...
class TiProductCategory(models.Model):
    ...
    parent_id = fields.Many2one("ti.product.category", string="Parent Category", index=True)
    child_ids = fields.One2many("ti.product.category", "parent_id", string="Subcategories")
...
class TiProductUidRule(models.Model):
    ...
    def generate_uid(self, category, spec_values=None, brand=None, vendor=None, consume_sequence=True):
        self.ensure_one()
        spec_values = spec_values or {}
        seq = self.sequence_id.next_by_id() if consume_sequence else self.sequence_id.number_next_actual
        product_type = {
            "raw_material": "RM",
            "bought_out": "BO",
            "finished_goods": "FG",
    ...
    def init(self):
        try:
            with self.env.cr.savepoint():
                ...
                self.env.cr.execute("CREATE INDEX IF NOT EXISTS ti_product_alias_normalized_trgm_idx ON ti_product_alias USING gin (normalized_name gin_trgm_ops)")
        except Exception:
            self.env.cr.execute("CREATE INDEX IF NOT EXISTS ti_product_alias_normalized_idx ON ti_product_alias (normalized_name)")
...
class ProductTemplate(models.Model):
    ...
    ti_subcategory_id = fields.Many2one("ti.product.category", string="PIS Subcategory", index=True)
    ti_brand = fields.Char(string="Brand/Make", index=True)
    ti_vendor_code = fields.Char(string="Vendor Code", index=True)
    ti_primary_vendor_id = fields.Many2one("res.partner", string="Primary Vendor", index=True)
    ti_material_details = fields.Text(string="Material Details")
    ...
    ti_keyword_ids = fields.One2many("ti.product.keyword", "product_tmpl_id", string="Keywords")
    ...
    ti_classification = fields.Char(compute="_compute_ti_classification", store=True)
    ...
    def init(self):
        ...
        try:
            with self.env.cr.savepoint():
                ...
                self.env.cr.execute("CREATE INDEX IF NOT EXISTS product_template_ti_keyword_trgm_idx ON product_template USING gin (ti_keyword_search_text gin_trgm_ops)")
                self.env.cr.execute("CREATE INDEX IF NOT EXISTS product_template_ti_spec_trgm_idx ON product_template USING gin (ti_spec_search_text gin_trgm_ops)")
        except Exception:
    ...
    def _compute_ti_search_fields(self):
        for product in self:
            ...
            keyword_parts = product.ti_keyword_ids.mapped("name")
            ...
            product.ti_keyword_search_text = normalize_text(" ".join([
                ...
                product.ti_brand or "",
                product.ti_vendor_code or "",
                product.ti_material_details or "",
                " ".join(spec_parts),
                " ".join(alias_parts),
                " ".join(keyword_parts),
            ]))

    @api.depends("ti_category_id", "ti_subcategory_id", "ti_product_type")
    def _compute_ti_classification(self):
        labels = dict(self.env["ti.product.category"]._fields["product_type"].selection)
        for product in self:
            parts = []
            if product.ti_product_type:
                parts.append(labels.get(product.ti_product_type, product.ti_product_type))
            if product.ti_category_id:
                parts.append(product.ti_category_id.name)
            if product.ti_subcategory_id:
                parts.append(product.ti_subcategory_id.name)
            product.ti_classification = " / ".join(parts)
    ...
    def create(self, vals_list):
        ...
        products = super(ProductTemplate, self.with_context(ti_skip_name_standard_check=True)).create(vals_list)
        products._ti_sync_default_code()
        products._ti_create_legacy_alias()
        products._ti_create_vendor_alias()
        products.with_context(ti_skip_name_standard_check=False)._check_ti_standard_name()
        return products
    ...
    def write(self, vals):
        ...
        if "ti_vendor_code" in vals:
            self._ti_create_vendor_alias()
    ...
    @api.constrains("name", "ti_uid", "ti_category_id", "ti_brand", "ti_spec_line_ids")
    def _check_ti_standard_name(self):
        if self.env.context.get("ti_skip_name_standard_check"):
            return
        for product in self.filtered(lambda item: item.ti_uid and item.ti_category_id and item.ti_governance_state in ("approved", "pending_review")):
            expected = build_standard_product_name(product.ti_category_id, product.ti_spec_line_ids.sorted("sequence"), product.ti_brand)
            if expected and product.name != expected:
    ...
    def _ti_create_vendor_alias(self):
        Alias = self.env["ti.product.alias"]
        for product in self.filtered("ti_vendor_code"):
            exists = Alias.search_count([
                ("product_tmpl_id", "=", product.id),
                ("alias_type", "=", "vendor_code"),
                ("name", "=", product.ti_vendor_code),
            ])
            if not exists:
                Alias.create({
                    "product_tmpl_id": product.id,
                    "alias_type": "vendor_code",
                    "name": product.ti_vendor_code,
                    "partner_id": product.ti_primary_vendor_id.id,
                })
